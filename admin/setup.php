...
/**
 * \file    dolireset/admin/setup.php
 * \ingroup dolireset
 * \brief   DoliReset setup page.
 */
...
require_once '../lib/dolireset.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "dolireset@dolireset"));
...
if ($action == 'updateMask') {
    ...
    foreach ($dirmodels as $reldir) {
        $file = dol_buildpath($reldir . "core/modules/dolireset/doc/pdf_" . $modele . "_" . strtolower($tmpobjectkey) . ".modules.php", 0);
        if (file_exists($file)) {
            $filefound = 1;
    ...
    if (!empty($tmpobjectkey)) {
        $constforval = 'DOLIRESET_' . strtoupper($tmpobjectkey) . "_ADDON";
    ...
    if ($ret > 0) {
        $tmpobjectkey = GETPOST('object');
        if (!empty($tmpobjectkey)) {
            $constforval = 'DOLIRESET_' . strtoupper($tmpobjectkey) . '_ADDON_PDF';
...
$head = doliresetAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "dolireset@dolireset");

// Setup page goes here
echo '<span class="opacitymedium">' . $langs->trans("DoliResetSetupPage") . '</span><br><br>';
                    ...
                    while (($file = readdir($handle)) !== false) {
                        if (strpos($file, 'mod_' . strtolower($myTmpObjectKey) . '_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
                            ...
                            if ($module->isEnabled()) {
                                ...
                                $constforvar = 'DOLIRESET_' . strtoupper($myTmpObjectKey) . '_ADDON';
